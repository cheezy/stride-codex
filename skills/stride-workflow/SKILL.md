...
3. **Partition the change set by commit state — this is where this port differs from the Claude Code original, deliberately.** That edition subtracts a claim-time dirty baseline (a snapshot of paths already dirty when the task was claimed, with per-path blob hashes) so a human's pre-claim edits are not counted as lines you wrote. **This port records no such baseline**, and `git blame` cannot separate the two — your edits and any edit already in the tree at claim time both read `Not Committed Yet`. Without a discriminator the change set is a strict *superset* of what you authored, which would produce a confidently-wrong **introduced** rather than an honest unknown. So partition on the one artifact-demonstrable line available:
   - **Responsible lines inside `git diff <base> HEAD`** — commits made after the base was recorded, therefore demonstrably yours.
   - **Responsible lines only in the uncommitted working-tree delta** (`git status --porcelain`, including untracked-new files) — indistinguishable from a pre-claim edit, therefore **not demonstrably yours**.

4. **Compare.**
   - Responsible lines are in the **committed range** → **introduced**. You wrote them, whatever the surrounding file's age.
     - *One narrow exception:* if they are in that range only because this task moved or reformatted them, and the faulty behaviour is shown older by a **repro against the base ref**, that is **discovered** — record the evidence. `git blame -w` is the secondary check and needs `-M`/`-C` to follow lines across files.
   ...
   - Responsible lines are in the **uncommitted delta only** → **discovered, labelled *provenance undetermined***, stating that this port cannot separate your uncommitted edits from edits already present at claim time.
...
| Critical finding, a reviewer ran, responsible lines in this task's **committed range** | **Introduced** → fail-closed: `testing_strategy.status` → `failed`, append `category: "testing"` / `severity: "critical"` to `issues[]`, bump `issue_counts.critical` + `issues_found`; fix, re-run the charter, re-review before completing |
...
| Critical finding, a reviewer ran, responsible lines in the **uncommitted delta only**, or the change set undeterminable, or the fault site unidentified | **Discovered**, labelled *provenance undetermined* rather than *pre-existing* → Step 7 (never block on a link you could not draw) |
