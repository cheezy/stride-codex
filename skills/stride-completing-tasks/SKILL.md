...
- **The automated path normally writes no artifact, and that is not a gap.** Nothing in the `explorer` agent's own contract instructs it to write a session file. The artifact convention (`.exploratory/sessions/`, `backlog.md`, `coverage.md`, `checks/`) lives in the `session` skill the agent composes *by reference*, and every skill that actually **executes** that convention is a command-skill Step 6.5 may not dispatch. So on the sanctioned path the prose summary is the **normal and complete record**, not a degraded fallback. Treat that as true of today's contract rather than a permanent guarantee.
