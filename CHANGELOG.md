...
- **The enforcement claim.** The Claude Code edition rests the escalation on its completion self-check's bidirectional verdict/issue rule. This port has no such rule — its self-check pairs verdicts with issues for `behaviour_test_matrix` rows and the nested `security_considerations.considerations[]` only, and the server does not backstop one. The policy now says so and rests on the **Step 6 review gate** acting on the appended Critical.
- **The provenance test.** The reference subtracts a claim-time dirty baseline so a human's pre-claim working-tree edits are not counted as lines the agent wrote. This port records no such baseline, and `git blame` cannot separate the two — both read `Not Committed Yet`. Without a discriminator the change set is a strict *superset* of what the agent authored, which would produce a confidently-wrong **introduced** and block a task that did not cause the defect. The test now partitions on **commit state**: lines inside `git diff <base> HEAD` are demonstrably the agent's; lines only in the uncommitted delta are not, and route to **discovered, labelled *provenance undetermined***. The base ref reads from this port's own `.stride/task-base-ref`, `HEAD~1` is barred from deciding provenance, and the **fix obligation is unconditional** on any Critical whatever the branch — so nothing ships broken while nothing blocks on a link that could not be drawn.
